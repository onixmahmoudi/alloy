...
describe("PHP Advanced Features", () => {
  it("should render modern PHP application with all features", () => {
      ...
      >
        ...
        <php.SourceFile path="Enums/UserStatus.php">
          <php.Namespace name="App\Enums">
            ...
            >
              <php.Method name="getLabel" visibility="public" returnType="string">
                return match($this) {"{"}
        ...
        <php.SourceFile path="Traits/Timestampable.php">
          <php.Namespace name="App\Traits">
            <php.Trait name="Timestampable">
              <php.Property name="createdAt" type="DateTime" visibility="private" readonly />
              <php.Property name="updatedAt" type="DateTime" visibility="private" />
        ...
        <php.SourceFile path="Models/User.php">
          <php.Namespace name="App\Models">
            <php.AttributeList 
              attributes={[
                { name: "Entity" },
                { name: "Table", arguments: [{ name: "name", value: '"users"' }] }
              ]} 
            />
            <php.Class name="User" final>
              ...
              />
              <php.Method name="getDisplayName" visibility="public" returnType="string">
    ...
    // Check composer.json
    const composerFile = result.find(f => f.path === "modern-php-app/composer.json");
    expect(composerFile).toBeDefined();
    expect(composerFile!.contents).toContain('"php": "^8.1"');

    // Check enum file
    const enumFile = result.find(f => f.path === "modern-php-app/src/Enums/UserStatus.php");
    ...
    // Check trait file
    const traitFile = result.find(f => f.path === "modern-php-app/src/Traits/Timestampable.php");
    expect(traitFile).toBeDefined();
    expect(traitFile!.contents).toContain("trait Timestampable");
    expect(traitFile!.contents).toContain("private readonly DateTime $createdAt");

    // Check class with attributes
    const userFile = result.find(f => f.path === "modern-php-app/src/Models/User.php");
    ...
    // Check interface
    const interfaceFile = result.find(f => f.path === "modern-php-app/src/Contracts/UserRepositoryInterface.php");
    expect(interfaceFile).toBeDefined();
    expect(interfaceFile!.contents).toContain("interface UserRepositoryInterface");

    // Check constants and functions
    const constantsFile = result.find(f => f.path === "modern-php-app/src/constants.php");

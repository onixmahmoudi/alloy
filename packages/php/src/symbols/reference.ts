...
import { PhpNamespaceScope } from "./php-namespace-scope.js";
import { PhpOutputSymbol } from "./php-output-symbol.js";
...
type ReferenceScope = PhpNamespaceScope | SourceFileScope;
...
export function ref(refkey: core.Refkey): () => string {
  ...
  const resolveResult = core.resolve<ReferenceScope, PhpOutputSymbol>(
  ...
  return core.memo(() => {
    ...
    if (
      sourceNamespace &&
      sourceNamespace.name !== targetNamespaceCtx!.qualifiedName
    ) {
      ...
      targetSrc!.addUse(targetDeclaration);
    }
